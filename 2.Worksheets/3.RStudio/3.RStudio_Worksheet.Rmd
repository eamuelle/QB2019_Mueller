...
rm(list=ls())
getwd()
setwd("~/GitHub/QB2019_Mueller/2.Worksheets/3.Rstudio")
...
col1 <- matrix(j, nrow = 5, ncol = 1, byrow = FALSE)
col2 <- matrix(z, nrow = 5, ncol = 1, byrow = FALSE)
k <- cbind(col1, col2)
...
> Answer 1: rnorm generates n observations of a normal distrubtion given a mean and standard deviation.
...
> Answer 2: The dimension of the matrix are 5x10.
...
> Answer 3:The strongest positive correlation is between TIN and TN. Most other pairs also have positive correlations. CHLA does not seem to be correlated with any of the other variables.
...
> Answer 4: Non-parametric tests should be used when the data is not normally distributed. The results of the correlation test are sensitive to whether you use parametric or non-parametric methods. For Pearson's method, the probability values corrected for multiple comparisons are slightly different than the uncorrected values which is important because as you do more comparison, the more likely errors are to occur.
...
> Answer 5: Total Nitrogen is a significant predictor of Zooplankton Biomass as shown by the low p-value. Thre residuals appear to be randomly distributed around zero. The Q-Q plot shows a reasonably linear relationship. The square root of the standardized residuals is mostly random and there do not appear to be many stand out cases in the residuals vs. legerage plot.
...
> Answer 6: Both Chydorus and Simocephallus were significant predictors of Zooplankton Biomass. Based on pairs correlations I was able to see that both Chydorus and Simocephallus were the species that were correlated with ZP. From there I ran linear regressions for each species to see that both are significant predictors of ZP. I then plotted the linear regression and relationship and generated confidence intervals for the relationships.


```{r}
zoopsbys <- read.table("data/zoops.txt", sep = "\t", header = TRUE)
zoopsbys <- zoopsbys[, c(3:11)]
ZP <- meso.num[,6]
zoops <- cbind(ZP, zoopsbys)
pairs(zoops)

corzoo <- cor(zoops)
print(corzoo)

fitregsimo <-lm(ZP~SIMO, data = zoops)
summary(fitregsimo)
plot(zoops$SIMO, zoops$ZP, ylim=c(0,10), xlim=c(0,3000), xlab = expression(paste("Simocephallus (", mu, "g/L)")), ylab = "Zooplankton Biomass (mg/L)", las = 1)
newSIMO <- seq(min(zoops$SIMO), max(zoops$SIMO), 10)
reglineSIMO <- predict(fitregsimo, newdata = data.frame(SIMO = newSIMO))
lines(newSIMO, reglineSIMO)
conf95SIMO <- predict(fitregsimo, newdata=data.frame(SIMO = newSIMO), interval = c("confidence"), level = 0.95, type = "response")
matlines(newSIMO, conf95SIMO[,c("lwr","upr")], type ="l", lty = 2, lwd = 1, col = "black")

fitregchyd <-lm(ZP~CHYD, data = zoops)
summary(fitregchyd)
plot(zoops$CHYD, zoops$ZP, ylim=c(0,10), xlim=c(0,9000), xlab = expression(paste("Chydorus (", mu, "g/L)")), ylab = "Zooplankton Biomass (mg/L)", las = 1)
newCHYD <- seq(min(zoops$CHYD), max(zoops$CHYD), 10)
reglineCHYD <- predict(fitregchyd, newdata = data.frame(CHYD = newCHYD))
lines(newCHYD, reglineCHYD)
conf95CHYD <- predict(fitregchyd, newdata=data.frame(CHYD = newCHYD), interval = c("confidence"), level = 0.95, type = "response")
matlines(newCHYD, conf95CHYD[,c("lwr","upr")], type ="l", lty = 2, lwd = 1, col = "black")
